...
namespace lab4_sharp
{
	class Rhombus
    {
		...
		{
			if (Length(A, B) != Length(B, C) && Length(A, B) != Length(C, D)&& Length(A, B) != Length(C, D) && Length(A, B) != Length(C, D))
			{
				throw new ArgumentException(message:"Coordinates should match Rhombus");
			}
			this.A = A;
			this.B = B;
			this.C = C;
	  ...
	  public static Rhombus operator*(Rhombus v,int d)
	  {
	    if(d == 0) 
		{ 
		  throw new ArithmeticException(message: "If you multiply on 0, it`ll not be a Rhombus"); 
		}
